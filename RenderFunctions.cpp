...
void draw_asset_rotated(SDL_Surface* surface, std::vector<Uint32> color, int x, int y, int pixel_size, int rotation) {

    int line_counter = 0;

    std::vector<Uint32> new_color(color.size());
    ...
    if (rotation == 1) {
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
                ...
                new_color[j * 16 + (i * -1 + 15)] = color[i * 16 + j];
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
                ...
                new_color[(i * -1 + 15) * 16 + j] = color[i * 16 + j];
    ...
    for (int i = 0; i < 256; i++) {
        if (new_color[line_counter] != 100000000) {
            draw_square(surface, (line_counter % 16) * pixel_size + x, ((line_counter - line_counter % 16) / 16) * pixel_size + y, pixel_size, new_color[line_counter]);
        }
        line_counter++;
    }
}
...
void draw_assets(SDL_Surface* surface, std::vector<std::vector<std::vector<Uint32>>> texture, int x, int y, int pixel_size, int team) {

    for (int i = 0; i < texture[0].size(); i++) {

        draw_asset(surface, texture[0][i], x + (i % 2) * pixel_size * 16, y + (i / 2) * pixel_size * 16, pixel_size);

    }

    for (int i = 0; i < texture[team].size(); i++) {

        draw_asset(surface, texture[team][i], x + (i % 2) * pixel_size * 16 + pixel_size * 32, y + (i / 2) * pixel_size * 16, pixel_size);
